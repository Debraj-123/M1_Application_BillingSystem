...
void generateBillody(char item[30],int qty, float price){
    printf("%s\t\t",item);
        printf("%d\t\t",qty);
        printf("%.2f\t\t",qty*price);
        printf("\n");
    ...
    while(contFlag == 'y'){
        ...
        switch(opt){
            ...
            for(int i=0;i<n;i++){
                ...
                printf("Pleas enter the unit price:\t");
            ...
            for(int i=0;i<ord.noOfItems;i++){
                generateBillody(ord.itm[i].item,ord.itm[i].qty,ord.itm[i].price);
            }
            ...
            while(fread(&order,sizeof(struct orders),1,fp)){
                ...
                for(int i=0;i<order.noOfItems;i++){
                    generateBillody(order.itm[i].item,order.itm[i].qty,order.itm[i].price);
